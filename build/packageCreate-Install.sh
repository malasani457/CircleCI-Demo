...
echo "Creating new package version for BrillioTest"
  Package_Version="$(sfdx force:package:version:create -p BrillioTest -d force-app -w 10 -v --targetdevhubusername SDevHub)"
  sleep 300 # We've to wait for package replication.

echo "Package installation es-base-objects.gt"
